...
datatype state = 
  State (fun_int: "entity function_interpretation") 
        (true_preds: "entity term atom set") 
        (false_preds: "entity term atom set")


definition nf_pred::"'ent function_interpretation \<Rightarrow> 'ent term atom \<Rightarrow> bool" where
  "nf_pred fi p = (\<forall>t \<in> (atom.ent p). nf_term fi t)"

fun n_state::"state \<Rightarrow> bool" where
  "n_state (State i t f) = (n_fi i \<and> (\<forall>p \<in> t. nf_pred i p) \<and> (\<forall>p \<in> f. nf_pred i p))"

context
  fixes s::state
  assumes n_state: "n_state s"
begin

abbreviation "nt \<equiv> normalise_term (fun_int s)"
abbreviation "ce \<equiv> check_eq (fun_int s)"

definition check_lt::"entity term \<Rightarrow> entity term \<Rightarrow> bool option" where
  "check_lt a b \<equiv> (case (nt a, nt b) of
    (Sym (entity.Num a'), Sym (entity.Num b')) \<Rightarrow> Some (a' \<le> b')
...
fun atom_sem::"entity term atom \<Rightarrow> bool option" where
  "atom_sem (Ent_Eq a b) = (if (ce a b) then Some True else None)"
| "atom_sem (Num_Le a b) = check_lt a b"
| "atom_sem (Num_Lt a b) = map_option (\<lambda>x. \<not>x) (check_lt b a)"
| "atom_sem (Pred p as) = 
    (let as' = map nt as in
      if      (Pred p as') \<in> (true_preds s) then Some True 
      else if (Pred p as') \<in> (false_preds s) then Some False 
      else None)"
end
...
abbreviation "at_sem \<equiv> atom_sem s"
text \<open>The variables in a symbol\<close>
fun sym_vars::"symbol \<Rightarrow> variable set" where
  "sym_vars (symbol.Var x) = {x}" 
| "sym_vars _ = {}"
...
fun inst_sym::"(variable \<Rightarrow> object) \<Rightarrow> time \<Rightarrow> symbol \<Rightarrow> entity" where
  "inst_sym f dur (symbol.Var x) = entity.Const (f x)"
| "inst_sym f dur (symbol.Const c) = entity.Const c"
...
| "ground_GD f (GD.ForAll (v, os) x) = (if (v \<notin> f_vars' x \<and> os \<noteq> []) then ground_GD f x else BigAnd (map (\<lambda>obj. ground_GD (f((symbol.Var v):=(entity.Const obj))) x) os))"
| "ground_GD f (GD.Exists (v, os) x) = (if (v \<notin> f_vars' x \<and> os \<noteq> []) then ground_GD f x else BigOr (map (\<lambda>obj. ground_GD (f((symbol.Var v):=(entity.Const obj))) x) os))"
