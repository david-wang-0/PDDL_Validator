...
datatype (ent: 'ent) of_upd = Fun_Upd upd_op func "'ent list" (ret_val: "'ent option")
...
datatype 'ent timed_effect = 
  DUpd_At_Start "'ent update"
| DUpd_At_End "'ent update"
...
datatype ('t) simple_action = Simple_Action
  ...
  (effect: "((variable \<times> type) list, 't) effect")

type_synonym ast_pre_GD = "((variable \<times> type) list, variable term atom) pre_GD"
type_synonym ast_simple_effect = "((variable \<times> type) list, variable term) effect"
...
datatype ast_action = 
  SA "var_num term simple_action"
  | DA "var_num_dur term durative_action"
