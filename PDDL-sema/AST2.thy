...
type_synonym wm = "time \<Rightarrow> state"
...
fun f_exp_val::"state \<Rightarrow> symbol term f_exp \<Rightarrow> rat option" where
    "f_exp_val M (f_exp.NFun fun as) = (case (nf_int M fun) of 
      Some f  \<Rightarrow> (let arg_vals = map (\<lambda>t. term_val M t) as
        in (if (list_all (\<lambda>x. x \<noteq> None) arg_vals) 
            then f (map the arg_vals) else None)) 
    | None    \<Rightarrow> None)"
  | "f_exp_val M (f_exp.Num n) = Some n"
  | "f_exp_val M (f_exp.Add x y) = (combine_options plus (f_exp_val M x) (f_exp_val M y))"
  | "f_exp_val M (f_exp.Sub x y) = (combine_options minus (f_exp_val M x) (f_exp_val M y))"
  | "f_exp_val M (f_exp.Mult x y) = (combine_options times (f_exp_val M x) (f_exp_val M y))"
  | "f_exp_val M (f_exp.Div x y) = (
      let 
        x' = f_exp_val M x;
        y' = f_exp_val M y
      in 
...
fun atom_val::"state \<Rightarrow> symbol term atom \<Rightarrow> bool option" where
  "atom_val s (Pred p as) = (let arg_vals = map (\<lambda>t. term_val M t) as
        in (if (list_all (\<lambda>x. x \<noteq> None) arg_vals) 
            then Some (Pred p (map the arg_vals)) 
            else None))"

fun form_sem::"state \<Rightarrow> e_form \<Rightarrow> bool" where
  "form_sem s GD.Bot = False"
| "form_sem s (GD.Atom a) = atom_sem s a" 



coinductive effect_sequence::"e_effect list \<Rightarrow> bool"
  and action_sequence::"e_action list \<Rightarrow> bool" 
  and true_cond_form::"e_cond_form \<Rightarrow> bool"
  and true_timed_form::"e_timed_form \<Rightarrow> bool"
  and true_form::"e_form \<Rightarrow> bool" where
  "effect_sequence []"
| "action_sequence []"
| "true_cond_form _"
| "true_timed_form _"
