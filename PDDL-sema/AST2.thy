...
locale Domain =
  fixes dom::"type \<Rightarrow> object set"
    and derived::"func \<times> variable list \<Rightarrow> e_form"
begin
  ...
  (* Do not forget derived predicates *)
  fun atom_val::"state \<Rightarrow> symbol term atom \<Rightarrow> bool option" 
    and form_val::"state \<Rightarrow> e_form \<Rightarrow> bool option" where
    "atom_val s (Pred p as) = (
      let arg_vals = map (\<lambda>t. term_val s t) as
      in (
        if (list_all (\<lambda>x. x \<noteq> None) arg_vals) 
        then (
          if ((Pred p (map the arg_vals)) \<in> true_preds s)
          then (Some True)
          else (
            if ((Pred p (map the arg_vals)) \<in> false_preds s)
            then Some False
            else (
  ...
  | "form_val s (GD.Not x) = map_option (\<lambda>x. x \<longrightarrow> False) (form_val s x)"
...
inductive upd_sem::"state \<Rightarrow> e_simple_update \<Rightarrow> state \<Rightarrow> bool" where
  "upd_sem s _ s"
  ...
  begin
    ...
    coinductive effect_sequence::"e_effect list \<Rightarrow> bool"
      ...
      and true_timed_form::"e_timed_form \<Rightarrow> bool" where
      "effect_sequence []"
    | "action_sequence []"
    | "true_cond_form _"
    | "true_timed_form _"
